...
namespace Microsoft.Teams.Apps.Grow.Bot
{
    ...
    public sealed class GrowActivityHandler : TeamsActivityHandler
    {
        ...
        private async Task HandleMemberAddedInTeamAsync(ITurnContext<IConversationUpdateActivity> turnContext)
        {
            ...
            bool operationStatus = await this.teamSkillStorageProvider.UpsertTeamSkillsAsync(teamEntity);
            if (!operationStatus)
            {
        ...
        private async Task HandleMemberRemovedInTeamScopeAsync(ITurnContext<IConversationUpdateActivity> turnContext)
        {
            ...
            // Delete team details if bot is uninstalled from a team.
            var result = await this.teamSkillStorageProvider.DeleteTeamSkillsAsync(teamId);

            if (!result)
            {
                this.logger.LogInformation($"Failed to delete the skills and team details for team: {teamId}");
            }
        }
    }
}